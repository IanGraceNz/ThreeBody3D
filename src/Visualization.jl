...
Create a 3D trajectory plot. Set `show=false` for tests or headless workflows.
Returns a Makie `Figure`.
"""
function plot_trajectory(result::SimulationResult;
                         bodycolors=(:red,:green,:blue), margin::Real=0.05,
                         show::Bool=true)
    length(bodycolors) == 3 || throw(ArgumentError("bodycolors must contain three colors."))
    _, points = _sample_solution(result, max(2, length(result.solution.t)))
