...
isempty(id_text) && error("A close-encounter regularised benchmark ID is required.")
decoded = ValidationFramework.decode_close_encounter_regularized_benchmark_id(Symbol(id_text))
definition = ValidationFramework.close_encounter_regularized_performance_definition(
...
measurement = run_performance_measurements(
    ValidationFramework.close_encounter_regularized_performance_operation(
