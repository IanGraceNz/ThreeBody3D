...
configuration = decode_core_tolerance_benchmark_id(benchmark_id)
...
measurement = run_performance_measurements(
    core_tolerance_performance_operation(configuration), policy,
)
report = build_performance_benchmark_report(
    definition, current_validation_environment(),
    ValidationFramework._core_validation_configuration(configuration), policy,
